...
for m=1:N
    vect_pr = (X[m,:].<0)'
    global match_row_idx = Int32[]
    mat_match = in.(vect_pr,idx_sign)
    for nn = 1:size(mat_match)[1]
        if all(mat_match[nn,:].==1)
            push!(match_row_idx,nn)
        end
    end
    one_hot = y[m,:]
    one_hot[match_row_idx[1]] = 1
    y[m,:] = one_hot
end
...
matwrite("loss_acc_4x4_oneHot.mat", Dict(
