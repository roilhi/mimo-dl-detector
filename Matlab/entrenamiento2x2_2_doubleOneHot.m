...
N = 1e5; % número de símbolos dataset de entrenamiento
...
for i=1:n_epocas
    ...
    %expZ2_1 = exp(Z2(1:output_size/2,:));
    %expZ2_2 = exp(Z2(output_size/2+1:end,:));
    %sum_expZ2_1 = repmat(sum(exp(Z2(1:output_size/2,:))),output_size/2,1);
    %sum_expZ2_2 = repmat(sum(exp(Z2(output_size/2+1:end,:))),output_size/2,1);
    %A2_first_rows = exp(Z2(1:output_size/2,:))./sum_expZ2_1;
    %A2_last_rows = exp(Z2(1:output_size/2,:))./sum_expZ2_2;
    %expA2 = exp(Z2);
    ...
    %expZ2V_1 = exp(Z2_V(1:output_size/2,:));
    %expZ2V_2 = exp(Z2_V(output_size/2+1:end,:));
    %sum_expZ2V_1 = repmat(sum(exp(Z2_V(1:output_size/2,:))),output_size/2,1);
    %sum_expZ2V_2 = repmat(sum(exp(Z2_V(output_size/2+1:end,:))),output_size/2,1);
    %A2V_first_rows = exp(Z2_V(1:output_size/2,:))./sum_expZ2V_1;
    %A2V_last_rows = exp(Z2_V(output_size/2+1:end,:))./sum_expZ2V_2;
